...
public class ItemSpawner : MonoBehaviour
{
    ...
    IEnumerator delaySpawn(float waitTime)
    {
        for (int i = 0; i < 3; i++)
        {
            ...
            GameObject SkillItemObj = Instantiate(skillitem, Return_RandomPosition(), Quaternion.identity);
            //SkillItem++;
        }
    ...
    public void dropskillItemandStealHealth(Transform transform)
    {
        player.GetComponent<PlayerStatus>().stealHealth(1.0f);

        player.GetComponent<PlayerStatus>().UpSkillStat(0.1f);
        //GameObject t_SkillItem = Instantiate(skillitem, transform.position, transform.rotation);
    }
